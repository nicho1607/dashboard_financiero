...
function CalendarPage({ transactions }) {
  ...
  // Agrupar transacciones por día del mes actual
  ...
  const getDayTotal = (day) => {
    const dayTrans = transactionsByDay[day] || [];
  ...
  for (let i = 0; i < firstDay; i++) {
    cells.push(<div key={`empty-${i}`} className="h-24"></div>);
  }
    ...
    cells.push(
      <div
        ...
        className={`h-24 p-2 border border-gray-100 dark:border-border-dark rounded-lg ${
          isToday ? "bg-indigo-50 dark:bg-indigo-900/20 border-indigo-300 dark:border-indigo-500" : "hover:bg-gray-50 dark:hover:bg-hover-dark"
        } transition-colors`}
      >
